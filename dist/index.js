...
async function cargoMetadata(cratePath, root) {
    ...
    await new Promise((resolvePromise, rejectPromise) => {
        const child = spawn(command, ['metadata', '--format-version', '1', '--no-deps', '--manifest-path', cratePath], { cwd: root, shell: false, stdio: ['ignore', 'pipe', 'inherit'] });
        ...
        child.on('exit', (code) => {
            if (code === 0)
                ...
                rejectPromise(new Error(`cargo metadata exited with code ${code}`));
        });
    });
...
export default function yrchatPlugin(options = {}) {
    ...
    return {
        ...
        async closeBundle() {
            ...
            if (!cratePackage)
                throw new Error(`Cargo package not found for ${cratePath}`);
            const cdylib = cratePackage.targets.find((target) => target.crate_types.includes('cdylib'));
            if (!cdylib)
                throw new Error('Plugin crate must expose a cdylib target');
            ...
            for (const required of ['memory', 'alloc', 'dealloc', 'plugin_handle']) {
                if (!exports.includes(required))
                    throw new Error(`Plugin WASM does not export ${required}`);
            }
