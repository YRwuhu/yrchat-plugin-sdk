...
const validManifest = {
  ...
  api_version: 1,
  kind: 'app',
  engines: { yrchat: '>=0.3.0 <1.0.0' },
  runtime: { frontend: 'yrchat-react@1' },
  provides: { 'game.context': '1.0.0' },
  requires: { 'ai.openai-compatible': '^1.0.0' },
  optional_requires: {},
  entry: 'component.wasm',
  ui: { entry: 'ui/index.html', title: 'Test', width: 800, height: 600, min_width: 320, min_height: 240, resizable: true },
  permissions: { ai_chat: false, ai_vision: false, character_context: false, network: [] },
};

test('validates API v1 manifests', async () => {
  const root = await mkdtemp(join(tmpdir(), 'yrchat-sdk-'));
  const path = join(root, 'manifest.json');
  await writeFile(path, JSON.stringify(validManifest));
  assert.equal((await validateManifest(path)).api_version, 1);
});
...
test('rejects unsupported API versions', async () => {
  ...
  for (const apiVersion of [2, 3]) {
    await writeFile(path, JSON.stringify({ ...validManifest, api_version: apiVersion }));
